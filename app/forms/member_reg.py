...
class UserRegForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['first_name'] = forms.CharField(max_length=100, widget=forms.TextInput(attrs={'class': 'form-control'}))
        self.fields['last_name'] = forms.CharField(max_length=100, widget=forms.TextInput(attrs={'class': 'form-control'}))
        self.fields['mail_address'] = forms.CharField(max_length=100, widget=forms.TextInput(attrs={'class': 'form-control'}))
        self.fields['tel'] = forms.CharField(max_length=12, widget=forms.TextInput(attrs={'class': 'form-control form-control'}))
        self.fields['postal_code'] = forms.CharField(max_length=7, widget=forms.TextInput(attrs={'class': 'p-postal-code form-control'}))
        self.fields['password'] = forms.CharField(max_length=20, widget=forms.PasswordInput(attrs={'class': 'form-control'}))
    ...
    class Meta:
        model = Users
        fields = (
            'first_name', 'last_name', 'mail_address', 'password', 'sex', 'tel', 'postal_code', 'prefectures', 'municipalities', 'address', 'apartment_mansion'
        )
    ...
    def clean_password(self):
        ...
        reg = re.search("(?=.*?[a-z])(?=.*?[A-Z])(?=.*?\d)[a-zA-Z\d]{6,}", password)
        if self.cleaned_data['password'] == '':
            pass
        elif reg == None :
            raise forms.ValidationError('パスワードは大小英字を含む8文字以上で入力してください')
        return password
...
class UserEditForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['sex'] = forms.ChoiceField(choices=SEX_CHOICE, widget=forms.Select(attrs={'class': 'form-control'}))
        self.fields['prefectures'] = forms.CharField(max_length=4, widget=forms.TextInput(attrs={'class': 'p-region form-control', 'readonly': 'readonly'}))
        self.fields['municipalities'] = forms.CharField(max_length=500, widget=forms.TextInput(attrs={'class': 'p-locality form-control', 'readonly': 'readonly'}))
        self.fields['address'] = forms.CharField(max_length=500, widget=forms.TextInput(attrs={'class': 'p-street-address form-control'}))
        self.fields['apartment_mansion'] = forms.CharField(max_length=100, required=False, widget=forms.TextInput(attrs={'class': 'p-extended-address form-control'}))
    ...
    class Meta:
        model = Users
        fields = (
            'sex', 'tel', 'postal_code', 'prefectures', 'municipalities', 'address', 'apartment_mansion'
        )
